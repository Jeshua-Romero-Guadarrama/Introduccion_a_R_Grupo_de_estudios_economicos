...
```{r esquema, echo=FALSE, out.width="80%", fig.cap="Etapas del proceso"}
...
También existen paquetes de `R` que permiten utilizar plataformas de ML externas, como por ejemplo `r citepkg("h2o", "https://github.com/h2oai/h2o-3/tree/master/h2o-r ")` o `r citepkg("RWeka")`.
...
Entre ellos podríamos citar `r citepkg("caret", "https://topepo.github.io/caret")`,`r citepkg("mlr3", "https://mlr3.mlr-org.com")` y `r citepkg("tidymodels", "https://www.tidymodels.org")`.
En la Sección \@ref(caret) se incluye una breve introducción al paquete `r citepkg("caret", "https://topepo.github.io/caret")` que será empleado en diversas ocasiones a lo largo del presente libro.
...
Entre ellos estarían R-Commander con el plugin FactoMineR (`Rcmdr`, `RcmdrPlugin.FactoMineR`) y `r citepkg("rattle", "https://rattle.togaware.com")`.
...
```{r polyfit, fig.cap="Muestra (simulada) y ajustes polinómicos con distinta complejidad."}
# Simulación datos
n <- 30
x <- seq(0, 1, length = n)
...
```{r polyfit2, fig.cap="Muestra con ajustes polinómicos con distinta complejidad y nuevas observaciones."}
...
```{r polyfitsim, fig.cap="Precisiones (errores cuadráticos medios) de ajustes polinómicos variando la complejidad, en las muestras empleadas en el ajuste y en nuevas observaciones (simulados)."}
nsim <- 100
set.seed(1)
grado.max <- 20
...
```{r obs-pred-plot, figure.caption="Gráfico"}
obs <- test$medv
...
```{exercise, label="train-validate-test"}
```
...
```{r ROC-curve, fig.cap="Curva ROC correspondiente al modelo de regresión logística."}
library(pROC)
...
```{r simdat100, fig.cap="Muestra simulada y tendencia teórica."}
...
```{r knnfit2, fig.cap="Predicciones con el método KNN y distintos vecindarios"}
...
```{r methods-caret, echo=FALSE, eval=is_html(), fig.cap="Listado de los métodos disponiles en `caret::train()`."}

mods <- getModelInfo()

model_info <- data.frame(model = names(mods), t(sapply(mods, function(x) { # x <- mods[[1]]
  num_param <- paste(as.character(x$param$parameter), sep = "", collapse = ", ")
  if(num_param == "parameter") num_param <- "None"
  type <- paste(sort(x$type), sep = "", collapse = ", ")
  libraries <- paste(x$library, sep = "", collapse = ", ")
  return(c(name = x$label, type = type, libraries = libraries, num_param = num_param))
})))

DT::datatable(model_info, rownames = FALSE,  
          colnames = c("`method`", "Modelo", "Tipo", "Librerías", "Hiperparámetros"),
