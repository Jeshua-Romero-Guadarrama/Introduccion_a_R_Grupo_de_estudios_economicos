...
contienen un modelo (de regresión) lineal. El paquete `r citepkg("Cubist", "https://topepo.github.io/Cubist")` es una evolución de M5 que incorpora un método *ensemble* similar a *boosting*.
...
Una de las más utilizadas son los *conditional inference trees* (Hothorn *et al.*, 2006)^[Otra alternativa es GUIDE (Generalized, Unbiased, Interaction Detection and Estimation; Loh, 2002).], implementada en la función `ctree()` del paquete `r citepkg("party")`.  
